...
void Chantier_Show(char *filename, float max_montant, dynlist_callback_t *precallback) {
	...
	if(header.count > 0) {
		...
		if(precallback == NULL) {
			list->callback.link  = (DYN_CALLBACK) Chantier_Handle_ShowFormated;
...
void Chantier_Delete(occupe_relation *relation, dynlist_node_t *selected) {
	...
	if(strcmp(selected->text, "(enregistrement supprim)") != 0 && DynamicList_Ask("Attention: cette opération est irréversible")) {
...
void Chantier_Handle_Delete(occupe_relation *relation) {
	...
	delcallback.link  = (DYN_CALLBACK) Chantier_Delete;
