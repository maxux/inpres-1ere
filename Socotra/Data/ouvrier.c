...
void Ouvrier_HandleDelete(occupe_relation *relation, dynlist_node_t *key) {
	...
	if(strcmp(key->text, "(enregistrement supprim)") != 0 && DynamicList_Ask("Attention: cette opération est irréversible")) {
...
void Ouvrier_Edit(ouvrier_t *position, index_node_t *ibuffer, short restrict, index_t *index) {
	...
	Fields_AppendNode(fields, "Naissance",  &position->d_naiss,     0,  FIELD_DATE, 1, Ouvrier_CompareDateNais, NULL);
...
short Ouvrier_CompareDateEnga(void *data, void *param, void *dummy) {
	short errcode = 1;
	date_t *input, *compare, today;
	time_t in_ts = 0, co_ts = 0, to_ts;
	...
	in_ts = Date_DateToTimestamp(input);
	co_ts = Date_DateToTimestamp(compare);
	to_ts = Date_DateToTimestamp(&today);

	if(to_ts - in_ts > -86400 && in_ts - co_ts > 0) {
		if(in_ts - co_ts < 409968000) {	/* = 13 years old (bissextile not managed) (60*60*24*365*13) */
			if(DynamicList_Ask("Engagement inférieur à 13 ans. Continuer ?"))
				errcode = 1;
			else
				errcode = 0;
		}
