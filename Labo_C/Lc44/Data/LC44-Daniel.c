...
/*
  Prochainement (todo list):
	...
	_ Recherche inverse num chantier/ouvrier
	_ Recherche pour supp
...
int main(int argc, const char *argv[]) {
	...
	char file_index[64], file_ouvrier[64], file_chantier[64], file_occupe[64];
	...
	Files_Init_Check(file_index, file_ouvrier, file_chantier, file_occupe, idx_ouvrier);

	/* Building PostCode */
	ZipCode_Init(&zipc);

	/* Relation entre les chantier/ouvrier/table relationnelle */
	relation.filename = file_occupe;	/* Fichier occupation  */
	relation.index    = idx_ouvrier;	/* Lien avec l'index   */
	relation.chantier = file_chantier;  /* Fichier chantier    */
	...
	DynamicList_AppendNode(menu, 10, "Afficher les occupations");
	DynamicList_AppendNode(menu, 11, "Ajouter une occupation");
	...
	do {
		...
		switch(choice->id) {
			...
			case 9:
				...
				Chantier_Handle_Delete(&relation);
				break;

			case 10:
				/* Affiche la table d'occupation */
				Occupation_ShowTable(file_occupe, idx_ouvrier, file_chantier);
				break;

			case 11:
				/* Ajoute un ouvrier/chantier dans les occupations */
				Occupation_Handle_Add(&relation);
				break;
