...
void Ouvrier_LoadIndex(char *filename, index_t *index) {
	...
	index->nodes = addr;
	if(fread(index->nodes, sizeof(index_node_t), MAX_ENTRY, fp) != MAX_ENTRY)
		File_Fail(filename);
...
void Ouvrier_SaveIndex(char *filename, index_t *index) {
	...
	if(fwrite(index->nodes, sizeof(index_node_t), MAX_ENTRY, fp) != MAX_ENTRY)
		File_Fail(filename);
...
void Ouvrier_BuildIndex(char *filename, index_t *index) {
	...
	while((read = fread(&temp, sizeof(ouvrier_t), 1, fp)) != 0) {
		if(temp.numregistre != -1)
			Index_AppendRaw(index, temp.nom, position);
...
void Index_AppendRaw(index_t *index, char *key, size_t offset) {
	index_node_t *node;
	size_t len;
...
index_t * Index_Create() {
	...
	idx->size  = 0;
	idx->nodes = (index_node_t *) malloc(sizeof(index_node_t) * MAX_ENTRY);
	if(idx->nodes == NULL)
		Exit_Fail();
